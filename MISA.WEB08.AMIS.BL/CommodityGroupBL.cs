...
namespace MISA.WEB08.AMIS.BL
{
    ...
    public class CommodityGroupBL : BaseBL<CommodityGroup>, ICommodityGroupBL
    {
        ...
        public Stream GetExport(string? keyword, string? sort)
        {
            ...
            List<CommodityGroup> employees = (List<CommodityGroup>)_commodityGroupDL.GetsDataExport(keyword, sort);
            var stream = new MemoryStream();
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            using (var package = new ExcelPackage(stream ?? new MemoryStream()))
            {
                // thêm 1 sheet vào file excel
                var sheet = package.Workbook.Worksheets.Add("DANH SÁCH NHÓM VẬT TƯ HÀNG HOÁ VÀ DỊCH VỤ");
                // style header
                sheet.Cells["A1:D1"].Merge = true;
                sheet.Cells["A1:D1"].Value = "DANH SÁCH NHÓM VẬT TƯ HÀNG HOÁ VÀ DỊCH VỤ";
                ...
                var employee = new CommodityGroup();
        ...
        public override ServiceResponse? CustomValidate(CommodityGroup commodityGroup)
        {
            ...
            if (commodityGroup.CommodityGroupID.ToString() == commodityGroup.ParentID)
            {
                validateFailures.Add("Nhóm vật tư hàng hoá không thể thuộc chính nó");
            }
        ...
        {
            var validateFailures = new List<string>();
            if (!_commodityGroupDL.CheckIncurred("InventoryItem", "CommodityGroupID", record.ToString()))
            {
